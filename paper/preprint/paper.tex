...
crucial that avoid penalizing implausibility that arises due to
mutability constraints. For any mutability-constrained feature \(d\)
this can be achieved by enforcing
\(\mathbf{x}^\prime[d] = \mathbf{x}[d]\) whenever the constraint is
violated, by adjusting either one to the other. Setting
\(\mathbf{x}[d] := \mathbf{x}^\prime[d]\) intuitively corresponds to
imposing user requirements on the target domain; from a Bayesian
perspective it can be understood as assuming a point mass prior for
\(p(\mathbf{x})\) with respect to feature \(d\)\footnote{For additional
  detail, see Section~\ref{sec-app-constraints} of the supplementary
  appendix.}.
...
\(y \in \{1,2\}\). Then for \(y^+ \sim p(y)=2\) we have:
...
Contrastive divergence pushes down (up) on the coefficient
\(\Theta[y^+,d]\) by the amount that feature \(d\) that is lower
(higher) for counterfactuals than samples. This has the intended effect
of aligning counterfactual explanations with the data, as discussed
above.

Suppose, for example, that feature \(d=1\) represents \emph{age} and our
model predicts the likelihood of credit default:
\(y \in \{1:=\text{'no default'},2:=\text{'default'}\}\). Then if
...
to the log odds of default, i.e.~the model learns to higher default risk
younger individuals. But if \emph{age} is immutable, then this
corresponds to penalizing younger individuals for implausibility that
arises due to a feature that they can change about themselves. This is
precisely what we want to avoid, and therefore we impose no penalty at
all.

\subsection{Gradient}\label{gradient}

\subsection{Full Training Loop}\label{full-training-loop}

Next, we defined the training objective and the counterfactual
generator. Finally, we initialize the optimization state.
