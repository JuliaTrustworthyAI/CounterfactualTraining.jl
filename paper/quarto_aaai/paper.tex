...
which valid counterfactual explanations derived for a model can be
deemed plausible with respect to the underlying data and global
actionability constraints (we refine this notion in Def.
...
\emph{LS} across both metrics, the distance-based \(\text{IP}\) (29\%)
and divergence-based \(\text{IP}^*\) (55\%).
...
Counterfactual outcomes in columns 1-3 are aggregated across bootstrap
samples and varying degrees of the energy penalty
\(\lambda_{\text{egy}}\) used for \emph{ECCo} at test time. Asterisks
(\(^*\)) in columns 1-3 indicate that difference is statistically
significant at 1\%-level. Standard errors for accuracy are bootstrapped
from the test set.}
