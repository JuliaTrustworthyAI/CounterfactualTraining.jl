...
- $\mathbf{x}^\prime_{t \leq T}$: a nascent counterfactual, defined as a counterfactual that has not yet converged.
- $\mathbf{x}^\prime_{T}$: a mature counterfactual, defined as a counterfactual that has either passed a threshold probability or exhausted all $T$ steps.
