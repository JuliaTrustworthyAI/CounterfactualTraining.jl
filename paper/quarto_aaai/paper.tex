...
the latter case, do not align with qualitative findings, which are much
more plausible for CT (Figure~\ref{fig-mnist}).
...
\caption{\label{fig-mnist}Explanations for \emph{MNIST} for BL (top) and
CT (bottom). First columns is a randomly chosen factual 0. Columns 2
through 5 are \emph{ECCo} counterfactuals for that factual in target
classes 1 to 4. Columns 6 trough 10 show integrated gradients averaged
over all test images in the corresponding classes (5-9).}
...
\emph{Adult} and \emph{MNIST} are not significant,
Figure~\ref{fig-mnist} demonstrates that CT does have the expected
effect: sensitivity to protected features as proxied by integrated
gradients is drastically reduced\footnote{Additional findings for
  integrated gradients are reported in the appendix.}. In the case of
\emph{Cred}, average costs increase, likely because any potential
benefits from protecting the \emph{age} are outweighed by the increase
in costs required for greater plausibility.
