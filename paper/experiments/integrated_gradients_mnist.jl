...
function test_MNIST(model1_path, model2_path; steps=50, baseline_type="zeros")
    ...
    for i in 0:9
        ...
        IG1 = calculate_average_contributions(model1, xs, ys;
            steps=steps, baseline_type="random")
        ...
        std_err_IG1 = round(std(IG1[:, protected_cols]) ./ sqrt(length(indices)), sigdigits=4)
        # Print results
        println("$mean_IG1 +/- $std_err_IG1")

        # Save a heatmap, rotated 90° left to obtain proper orientation
        savefig(heatmap(mapslices(rotl90, IG1, dims=[1, 2]),
            aspect_ratio=:equal, seriescolor=:thermal, 
            axis=false,
            ticks=nothing, border=false,
            colorbar=false,
            framestyle=:none,
            margin=0px, padding=0px, size=(600, 600)),
        "data/mnist1_$i.png")
        
        # -------------------------

        # Get average contributions for Model 2
        IG2 = calculate_average_contributions(model2, xs, ys;
            steps=steps, baseline_type="random")
        # Normalize into 0-1 range
        ...
        # Calculate standard error only over protected columns
        std_err_IG2 = round(std(IG2[:, protected_cols]) ./ sqrt(length(indices)), sigdigits=4)
        # Print results
        println("$mean_IG2 +/- $std_err_IG2")

        # Save a heatmap, rotated 90° left to obtain proper orientation
        savefig(heatmap(mapslices(rotl90, IG2, dims=[1, 2]),
            aspect_ratio=:equal, seriescolor=:thermal, 
            axis=false,
            ticks=nothing, border=false,
            colorbar=false,
