...
\tightlist
\item
  To what extent does the counterfactual training objective (as defined
...
explainability (Def. \ref{def-explainability}). To this end, we focus
...
smaller average costs (\(\ell_1\)-norm) to individuals. To extend this
to the non-linear case, we test the effect of imposing mutability
constraints empirically for our synthetic data using the same evaluation
scheme as above. The final row in Table~\ref{tbl-main} reports the
average reduction in costs for CT compared to the baseline, when
