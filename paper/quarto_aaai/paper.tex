...
improved ``explainability'' of a model can arise as a consequence of
regularization techniques aimed at other training objectives such as
...
of models. In high-level terms, we posit that the explainability of a
model depends on the extent to which valid explanations derived for this
model are also deemed plausible with respect to the underlying data and
the global actionability constraints (see Def.
\ref{def-explainability}). To the best of our knowledge, Counterfactual
...
We say that $\mathbf{M}_\theta$ is \textbf{explainable} to the extent that faithfully generated counterfactuals are also plausible and actionable. We define these properties as follows:
