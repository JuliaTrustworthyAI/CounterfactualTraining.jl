...
features, but this is not guaranteed if the assumptions in Prp.
...
\emph{Plausibility is costly.} As noted elsewhere
\citep{altmeyer2024faithful}, more plausible counterfactuals are
inevitably more costly. Thus, while CT improves plausibility and
robustness, it can impact average costs and validity as cheap,
implausible and adversarial explanations are removed from the solution
space.

\emph{CT increases the training times.} Like contrastive and robust
...
Secondly, it is worth extending the analysis done here to additional CE
generators including those that do not rely on computationally costly
gradient-based optimization. This would reduce training costs while
potentially maintaining benefits of CT.
