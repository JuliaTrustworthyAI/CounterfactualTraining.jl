...
\emph{Polyjuice}, a general-purpose CE generator for language models,
and demonstrate that the augmentation of training data with
\emph{Polyjuice} improves robustness in a number of tasks.
...
\(\mathbf{x}_{\text{AE}}^\prime\) that have not yet converged. As we
...
\begin{aligned}
&\min_\theta \text{yloss}(\mathbf{M}_\theta(\mathbf{x}),\mathbf{y}) + \lambda_{\text{div}} \text{div}(\mathbf{x}^+,\mathbf{x}_{\text{CE}}^\prime,y;\theta) \\+ &\lambda_{\text{adv}} \text{advloss}(\mathbf{M}_\theta(\mathbf{x}_{\text{AE}}^\prime),\mathbf{y}) + \lambda_{\text{reg}}\text{ridge}(\mathbf{x}^+,\mathbf{x}_{\text{CE}}^\prime,y;\theta)
\end{aligned}
...
\(\mathbf{x}_{\text{AE}}^\prime\). Finally, \(\text{ridge}(\cdot)\)
...
counterfactuals (negative samples) \citep{du2019implicit}. As the
...
integrated gradients \citep{sundararajan2017ig}. Finally, for predictive
...
tabular datasets: \emph{Adult} (Census data) of
...
\citet{kaggle2011give}. Finally, for the convenience of illustration, we
use the 10-class \emph{MNIST} \citep{lecun1998mnist}.
...
Figure~\ref{fig-acc}. The perturbation size, \(\varepsilon\in[0,0.1]\),
...
and thus meaningful to their recipients. In turn, our work highlights
