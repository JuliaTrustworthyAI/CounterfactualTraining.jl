...
Definition~\ref{def-explainability} is the same as in
...
purposes, we let \(T\) vary and consider the counterfactual search to be
...
generate samples, \cite{grathwohl2020your} rely on Stochastic Gradient
...
the counterfactuals get more plausible
(Definition~\ref{def-explainability}) during training, these opposing
effects gradually balance each other out \cite{lippe2024uvadlc}.
...
to explainability (Definition~\ref{def-explainability}). To this end, we
...
CEs for the training samples for free, which may be beneficial in many
