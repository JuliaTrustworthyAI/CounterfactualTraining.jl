...
@grathwohl2020your observe that any classifier can be re-interpreted as a joint energy-based model (JEM) that learns to discriminate output classes conditional on inputs and generate inputs. They show that JEMs can perform well at both tasks by directly optimizing the joint distribution $\log p_\theta(x,y)=\log p_\theta(y|x) + \log p_\theta(x)$. The first factor 
The distribution over inputs is learned via contrastive divergence: 

Considered through the lens of explainability, JEMs learn plausible explanations.
