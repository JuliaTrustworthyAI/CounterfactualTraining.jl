...
gen = NativeGenerator()
obj = FullObjective()
opt_state = Flux.setup(Flux.Adam(1e-3), model_full)
...
opt_state_vanilla = Flux.setup(Flux.Adam(1e-3), model_vanilla)
...
plt = plot!(1:length(t_vanilla), t_vanilla; label="Vanilla objective", lw=2)
```
