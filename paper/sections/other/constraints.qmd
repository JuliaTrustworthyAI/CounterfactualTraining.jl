...
function plot_db(model, x; _xlab="Existing Debt", _ylab="Age")
    ...
    plt = Plots.scatter(ce_data, label=["1=Default" "2=No Default"], xlab=xlab, ylab=ylab, title="Iteration $(_itercount)")
    Plots.abline!(plt, db.slope, db.intercept; lw=5, label="Dec. Boundary")

    # Counterfactuals:
    if !any(isnothing.(x))
        yhat = [argmax(y) for y in eachcol(model(x))]
        Plots.scatter!(x[1,:], x[2,:], label=["CE (y⁺=1)" "CE (y⁺=2)"], ms=15, shape=:star, color=yhat, group=yhat)
    end
    display(plt)
...
# Data:
specs = [
    (VanillaObjective(needs_ce=true), ["both", "both"]),
    (EnergyDifferentialObjective(lambda=[1.0,1.0,0.0]), ["both", "both"]),
    (EnergyDifferentialObjective(lambda=[1.0,1.0,0.0]), ["both", "none"]),
]
```
