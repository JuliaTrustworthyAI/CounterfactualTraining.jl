...
get_table_inputs(df, "value"; alpha=0.9, byvars="lambda_energy_exper", backend=Val(:latex)) |>
    inputs -> tabulate_results(inputs; label="tab:lin_sep-first_run", title="Results for Linearly Separable data.")
```
...
```{julia}
tabulate_results(df, "value"; alpha=0.9, byvars="lambda_energy_exper", backend=Val(:latex))
```
