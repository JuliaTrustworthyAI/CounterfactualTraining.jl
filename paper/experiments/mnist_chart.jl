...
gen = CTExperiments.GeneratorParams(lambda_energy=1.0) |>  # get generator
...
while !both_predict_factual
    ...
    both_predict_factual = predict_label(models[1], data, x)[1] == predict_label(models[2], data, x)[1] == factual
end
...
for M in models
    ...
    for target in chosen_digits[2:end]
        img = generate_counterfactual(x, target, data, M, gen; convergence=conv) |> 
