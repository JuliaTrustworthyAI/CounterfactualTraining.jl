...
(\citeproc{ref-altmeyer2023explaining}{Altmeyer, van Deursen, and Liem
...
mutated in the allowed direction, if at all.
...
worst, most likely because it attempts to explain the decisions of a VAE
that is an imperfect surrogate of the original model. Third, increasing
\(T\), the maximum number of steps, generally yields better outcomes
because more CEs can mature in any iteration. Fourth, the impact of
\(\tau\), the required decision threshold is more difficult to predict.
On ``harder'' datasets it may be difficult to satisfy high \(\tau\) for
any given sample (i.e., also factuals) and so increasing this threshold
does not seem to correlate with better outcomes.
...
combined with high values for Equation~\ref{eq-obj} penalties, this
...
instabilities can be largely counteracted with strong regularization, in
particular through \(\lambda_{\text{reg}}\) penalty on the energy.

\textbf{\emph{CT is sensitive to hyperparameter selection.}} As
discussed in Section~\ref{sec-hyperparameters}, our method requires
careful selection of the hyperparameters. All experiments relied on
grid-search tuning but we recognize that other algorithms may be more
efficient. Notably, counterfactual training is iterative which makes a
variety of methods applicable, including Bayesian (e.g.,
...
\textbf{\emph{CT increases the training time of models.}}

\textbf{\emph{Immutable features may have proxies.}}
