...
# Detailed table with varying levels of hyperparameter:
df = final_table(res_dir; agg_further_vars=[["run"], ["run"]])
inputs = get_table_inputs(df, nothing; backend=Val(:latex))
formatters = (
    PrettyTables.ft_round(1,findall(.!contains.(names(df),"Acc"))),
    PrettyTables.ft_round(2,findall(contains.(names(df),"Acc")))
)
for fname in ["paper/quarto_aaai/tables/main_detailed.tex","paper/preprint/tables/main_detailed.tex"]
    tabulate_results(
        inputs; 
        table_type=:tabular, 
        save_name=fname, 
...
df = final_table(res_dir, total_uncertainty=false)
inputs = get_table_inputs(df, nothing; backend=Val(:latex))
formatters = (
    PrettyTables.ft_round(1,findall(.!contains.(names(df),"Acc"))),
    PrettyTables.ft_round(2,findall(contains.(names(df),"Acc")))
)
for fname in ["paper/quarto_aaai/tables/main_marginalized.tex","paper/preprint/tables/main_marginalized.tex"]
    tabulate_results(
        inputs; 
        table_type=:tabular, 
        save_name=fname, 
