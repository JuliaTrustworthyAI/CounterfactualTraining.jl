...
```{julia}
plt = CTExperiments.plot_performance(res_dir, eps=range(0.0, 0.1, length=10) |> collect, adversarial=true, byvars=["objective", "eps"]) |>
    plt -> draw(plt, figure=(size=(800, 190),), axis=(yticks=[0.0,0.5,1.0], limits=(nothing, (0,1)), xticklabelsvisible=false, xticksvisible=false))
save("paper/figures/acc.png", plt; px_per_unit=3)
```
...
df_mmd.measure .= y
df = innerjoin(df_mtbl, df_ip, df_mmd; on=:dataset)
```
