...
We find that CT can substantially improve the quality of explanations
learned by models without sacrificing predictive performance. Test
accuracy for CT is virtually identical to the baseline for \emph{Adult},
...
in test accuracy of 2, 5 and 15 percentage points, respectively. In this
context, we also note that we have not optimized the models (very small
networks) for predictive performance at all.

\begin{table}

\caption{\label{tbl-main}Key performance metrics across all datasets.
\textbf{Plausibility}: Rows 1-5 show the percentage reduction in
implausibility via Equation~\ref{eq-impl-dist} for varying degrees of
the energy penalty used for \emph{ECCo} at test time; row 6 shows the
reduction in implausibility via Equation~\ref{eq-impl-div} and
aggregated across all test specifications. \textbf{Accuracy} (rows 7-8):
test accuracies for models trained with CT and the baseline.
\textbf{Actionability} (row 9): average reduction in costs when imposing
...
\cite{spooner2021counterfactual}), but there is little consensus how to
...
Bayesian or gradient-based optimization applicable (see
\cite{bischl2023hyperparameter}).
