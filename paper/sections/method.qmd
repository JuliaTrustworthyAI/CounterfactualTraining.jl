...
Let $\mathbf{M}_\theta(\mathbf{x})=\Theta\mathbf{x}$ denote a linear classifier with $y\in\{1,...,K\}=\mathcal{K}$ and $\mathbf{x} \in \mathbb{R}^D$. If we assume multivariate Gaussian class densities with common diagonal covariance matrix $\Sigma_k=\Sigma$ for all $k \in \mathcal{K}$, then protecting an immutable feature from the contrastive divergence penalty [@eq-div] will result in lower classifier sensitivity to that feature, provided that at least one other feature is mutable.
