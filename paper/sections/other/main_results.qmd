...
As IG outputs are not bounded (i.e., they are arbitrary real numbers), it becomes a challenge to meaningfully compare IG outputs of different models — ones that are trained conventionally, and ones that underwent counterfactual training. For our purposes, we observe that the direction of contribution (sign) does not matter. If protecting a feature flips the sign of the corresponding output, then CT „overcorrected” for this feature, but as long as the magnitude of its contribution decreases, then CT achieved its objective according to our formulation. Thus, we focus on the magnitude of IG outputs in the evaluation. Next, to accommodate for variable ranges of outputs, we standardize the outputs. 

Let $\mathbb{g}_d$ denote the estimated IG for feature $d$. Then in the case of 2D synthetic datasets we find that taking the absolute value of the outputs, $|\mathbb{g}_d|$, and then dividing them by a $\max(\mathbb{g}) - \min(\mathbb{g})$ term allows us to make the most meaningful comparison. In the case of real-world datasets we choose to normalize the values to a 0-1 range instead.

We compare the (average) sensitivity to the features that were protected for CT models. We again use bootstrapping (100 rounds, 2500 samples per round) to establish the significance of our results. These are presented in @tbl-ig.

We observe substantial and statistically significant improvements for seven out of nine datasets. For the synthetic datasets we observe reduction in sensitivity to the protected feature in 3 out of 4 cases. The only dataset negatively impacted by CT — albeit with highly variable results — is *Moon*, which clearly violates the assumptions in Proposition 3.1. For the real-world datasets, the reductions are over two-fold for *Adult* and *CH* and almost three-fold for *MNIST*. In case of *Cred* we can fully prevent the CT model from considering *age* as a factor in classification. Only for *GMSC* we observe negative impacts of CT which we believe is due to any or all of the following: a) data assumptions are violated; b) the impact of other components of the CT objective outweighs expected effects of feature protection; or c) the default baseline choice applied consistently to all datasets is not appropriate for GMSC.
...
3. From a philosophical perspective on algorithmic recourse, validity in and off itself is not a sufficient desideratum for counterfactuals. In fact, @ven2020philosophical propose introducing an upper bound on costs of the flipset (i.e. the set of valid CE), arguing that valid but highly costly counterfactuals are not useful to individuals in practice. In a similar fashion, it could be argued that there should be an upper bound on the implausibility of counterfactuals in the flipset.
