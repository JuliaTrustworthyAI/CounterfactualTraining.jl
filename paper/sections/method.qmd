...
Since actionability constraints are binding at test time, we should also impose them when generating $\mathbf{x}^\prime$ during each training iteration to align model representations with user requirements. Through their effect on $\mathbf{x}^\prime$, both types of constraints influence model outcomes through @eq-div. Here it is crucial that avoid penalizing implausibility that arises due to mutability constraints. For any mutability-constrained feature $d$ this can be achieved by enforcing $\mathbf{x}^\prime[d] = \mathbf{x}[d]$ whenever the constraint is violated, by adjusting either one to the other. Setting $\mathbf{x}[d] := \mathbf{x}^\prime[d]$ intuitively corresponds to imposing user requirements on the target domain; from a Bayesian perspective it can be understood as assuming a point mass prior for $p(\mathbf{x})$ with respect to feature $d$^[For additional detail, see @sec-app-constraints of the supplementary appendix.]. 

Protecting immutable features in this way effectively forces the model to instead seek plausibility with respect to the remaining features. 