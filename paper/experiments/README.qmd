...
The charts in Figures 3 and 4 of the paper can be reproduced using the following script: [paper/experiments/mnist_chart.jl](mnist_chart.jl).

### Table I

Table I showing the aggregated results for our main metrics of interest is produced in [paper/sections/other/main_results.qmd](../sections/other/main_results.qmd). Integrated gradients were computed using [paper/experiments/integrated_gradients_mnist.jl](integrated_gradients_mnist.jl).

### Figure 5

The chart in Figure 5 of the paper can be reproduced using the following script: [paper/experiments/ar_plot.jl](ar_plot.jl).

### Table II

The results for integrated gradients presented in Table II can be reproduced using [paper/experiments/run_integrated_gradients.jl](run_integrated_gradients.jl). The script uses multi-processing and can be executed as follows:

```
mpiexecjl --project=paper/experiments -n 5 julia paper/experiments/run_integrated_gradients.jl --nrounds=100 --nsamples=2500
```

### Table III

The results shown in Table III are produced in [paper/sections/other/main_results.qmd](../sections/other/main_results.qmd).

### Figure 6

The chart in Figure 6 of the paper can be reproduced using the following scripts: [paper/experiments/final_table_ablation.jl](final_table_ablation.jl) and then [paper/experiments/ablation_plot.jl](ablation_plot.jl).
