...
CT adapts gradient-based CE search already in training to generate
\(\mathbf{x}^\prime\), the perturbed versions of the factual samples
\(\mathbf{x}\). We use the term \emph{nascent} to denote counterfactuals
for which the search has not converged, \(\mathbf{x}_{t\leq T}^\prime\).
Initially, when the perturbations are small, nascent counterfactuals
serve as AEs. Over multiple iterations, they move towards the decision
boundary and turn into \emph{mature} counterfactuals
\(\mathbf{x}_T^\prime\) when they cross into the target class (becoming
CEs) or when \(t > T\).
