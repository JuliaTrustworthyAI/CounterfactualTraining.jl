...
**Example**: Let $f_\theta(x)=\theta^{T}x$ be a linear classifier. Suppose that featue $x_1$ is immutable (e.g. 'gender' or 'faith'). 

$$
\begin{aligned}
\nabla_\theta \ell_{\text{gen}}(x^\prime,x;\theta) &= \nabla_\theta ( \theta^{T}x[t] - \theta^{T}x^\prime[t]) \\
\frac{\partial \ell_{\text{gen}}}{\partial\theta_1}(x^\prime,x;\theta)&=x_1 - x_1^\prime
\end{aligned}
$$
