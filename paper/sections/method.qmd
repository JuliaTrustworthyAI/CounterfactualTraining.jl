...
Let $\mathbf{x}_t^\prime$ for $t=0,...,T$ denote a counterfactual generated through gradient descent over $T$ iterations as originally proposed by @wachter2017counterfactual. In broad terms, generating CEs using gradient descent entails solving some form of an objective that balances (1) the classification loss $\text{yloss}(\mathbf{M}_\theta(\mathbf{x}),\mathbf{y})$, and (2) one or more penalty terms $\lambda_{i}\text{cost}_{i}(\cdot)$. The exact specification of these penalties induces various properties in the counterfactual outcomes, and tends to be the key feature that distinguishes various gradient-based "generators" or "explainers" (CE algorithms) from the literature [@altmeyer2023explaining].

For our purposes, we let $T$ vary and consider the search to be converged as soon as the predicted probability for the target class has reached a pre-determined threshold, $\tau$: $\mathcal{S}(\mathbf{M}_\theta(\mathbf{x}^\prime))[y^+] \geq \tau$, where $\mathcal{S}$ is the softmax function.

CT leverages gradient-based CE search already in training to produce perturbed versions of the factual samples $\mathbf{x}$. Initially, until $\tau$ is reached or when $t\leq T$, generated samples $\mathbf{x}_t^\prime$ serve as adversarial examples; we term them *nascent* counterfactuals $\mathbf{x}_{t\leq T}^\prime$. Over multiple training iterations up to time $T$, these nascent counterfactuals move towards the decision boundary and turn into *mature* counterfactuals $\mathbf{x}_T^\prime$ when they cross into the target class or when $t > T$.
...
`\noindent`{=latex} Proposition \ref{prp-mtblty} assumes independent features, which raises a concern about the effect of protecting immutable features in the presence of proxies. We address it in @sec-discussion.
