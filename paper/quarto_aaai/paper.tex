...
to semantically meaningless explanations. Likewise, previous work of
...
including those introduced above (faithfulness, plausibility and
...
First, we look at hyperparameters that guide the gradient-based
counterfactual search. Increasing \(T\), the maximum number of steps,
generally yields better outcomes because more CEs can mature in each
training epoch. The impact of \(\tau\), the required decision threshold
is more difficult to predict. On ``harder'' datasets it may be difficult
to satisfy high \(\tau\) for any given sample (i.e., also factuals) and
so increasing this threshold does not seem to correlate with better
