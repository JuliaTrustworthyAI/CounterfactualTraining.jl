...
In other words, contrastive divergence will push down (up) on the coefficient $\theta[d]$ corresponding to a feature $d$ that is lower (higher) for counterfactuals than samples. This has the intended effect of aligning counterfactual explanations with the data, as discussed above. 

Crucially though, we want to avoid penalizing implausibility that arises due to mutability constraints. Suppose, for example, that feature $d$ represents *age* and our model predicts the likelihood of credit default. 
